...
def wannaUpSelect(user_rate_summary,
    ...
    wanna_up = wanna_up[wanna_up.asset_type == 1].drop("asset_type", axis=1)
...
def wannaDownSelect(user_rate_summary,
    ...
    wanna_down = match_record[(match_record.userid.isin(down_users)) &
                              ...
                              (match_record.amount >= lowAmount)]
    wanna_down = pd.merge(wanna_down, asset_type_data, on="asset_id", how="left")
    wanna_down = wanna_down[wanna_down.asset_type == 1].drop("asset_type", axis=1)
...
z3 = "select asset_id,asset_type from ast_matching_asset_group where yn=0 and status!=3"

match_record = read_sql_query(sql=z0, con=asset_con)
asset_data = read_sql_query(sql=z1, con=asset_con)
asset_type_data = read_sql_query(sql=z3,con=asset_con)
regular_info = read_sql_query(sql=z2, con=cash_con)


print("sql finished!")
# path = "F:/20170808data/"
# match_record.to_csv(path+"match_record.csv",index=False)
# asset_data.to_csv(path+"asset_data.csv",index=False)
# regular_info.to_csv(path+"regular_info.csv",index=False)
# asset_type_data.to_csv(path+"asset_type_data.csv",index=False)
...
# asset_type_data = pd.read_csv(path+"asset_type_data.csv)

# 创建囊括所有资金类型的预期利率词典
num = np.arange(1, 12.5, 0.5)
exp_rate = [6.1, 6.15, 6.2, 6.25, 6.29, 6.3, 6.3, 6.35, 6.4, 6.5, 6.59, 6.6, 6.6, 6.65, 6.7, 6.8, 6.89,
     6.9, 6.9, 6.9, 6.9, 6.95, 6.99]
regular_rate = pd.DataFrame({"num": num, "exp_rate": exp_rate})
n = 200
rate_dict = {"current": 5.8, "tPlus": 6.0, "regular": regular_rate}
# 输出用户利率现状
user_rate_summary = getRateSummary(match_record, asset_data, regular_info, rate_dict)

# 选择需要向上调整的资金队列
up_result = wannaUpSelect(user_rate_summary, match_record, asset_data,asset_type_data,lowDiffRegRate=-0.1,lowDiffTotalRate=-0.1)
